...
| D-034 | ACCEPTED | Retain the D-031 floor of at least 15 Unicode code points without Quorum plaintext validation. Prefer native code-point enforcement; a provider-native encoded-length control is acceptable only when its configured threshold conservatively guarantees the same floor for every untransformed input: at least 29 UTF-16 code units or at least 57 UTF-8 bytes, while accepting at least 64 ASCII characters. | The owner delegated this conservative choice on 2026-07-27. A03 must prove the provider's actual unit, ordering versus normalization, 14-code-point maximum-width negative control, 15-code-point and 64-ASCII positive controls, no silent truncation, and consistent normalization. Any length-expanding transform before measurement fails unless a stronger bound is proven. This resolves the decision blocker but does not accept a provider or pass G3. R-007/R-008 remain historical evidence; retesting requires a new exact configuration result. |

...
| PASS-02 | The single-factor general-user POC password input MUST require at least 15 Unicode code points, accept at least 64 (target maximum 128), allow Unicode/space as supported, permit password managers and paste, avoid composition rules, never silently truncate, and reject common/breached values through a maintained privacy-preserving mechanism. Native code-point enforcement is preferred. Under D-034, provider-native encoded-length enforcement is allowed only with a conservative threshold that guarantees this floor for every untransformed input: at least 29 UTF-16 code units or at least 57 UTF-8 bytes, while accepting at least 64 ASCII characters. The accepted framework's documented measurement unit, ordering versus normalization, limits, and failure behavior are tested configuration; a length-expanding transform before measurement requires a separately proven stronger bound. Quorum does not add ad hoc plaintext transformations. |
